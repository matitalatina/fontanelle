import { getBicycleParkingsFromDB } from "@/lib/bicycleParking";
import { getStationsFromDB } from "@/lib/stations";
import { getToiletsFromDB } from "@/lib/toilets";
import { NextRequest } from "next/server";

export async function GET(request: NextRequest) {
  const searchParams = request.nextUrl.searchParams;
  const gh5Param = searchParams.get("gh5");
  const overlaysParam = searchParams.get("overlays");

  if (!gh5Param) {
    return Response.json({ error: "Missing gh5 parameter" }, { status: 400 });
  }

  const gh5List = gh5Param.split(",");
  const overlays = overlaysParam
    ? overlaysParam.split(",")
    : ["stations", "toilets", "bicycleParkings"];

  try {
    const promises = [];
    const results: any = {
      stations: [],
      toilets: [],
      bicycleParkings: [],
    };

    if (overlays.includes("stations")) {
      promises.push(
        getStationsFromDB(gh5List).then((data) => (results.stations = data))
      );
    }

    if (overlays.includes("toilets")) {
      promises.push(
        getToiletsFromDB(gh5List).then((data) => (results.toilets = data))
      );
    }

    if (overlays.includes("bicycleParkings")) {
      promises.push(
        getBicycleParkingsFromDB(gh5List).then(
          (data) => (results.bicycleParkings = data)
        )
      );
    }

    await Promise.all(promises);
    return Response.json(results);
  } catch (error) {
    console.error("Error fetching entities:", error);
    return Response.json(
      { error: "Failed to fetch entities" },
      { status: 500 }
    );
  }
}
